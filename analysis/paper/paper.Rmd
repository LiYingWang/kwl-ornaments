...
ornaments_intact_glass <-
  ornaments_period %>% 
  filter(Condition %in% c('complete', 'near-complete'),
         Categories == 'Glass bead') %>% 
  mutate(Length = as.numeric(`Length(mm)`),
         Thick = as.numeric(`Thick(mm)`),
         Width = as.numeric(`Width(mm)`)) %>% 
  rowwise() %>% 
...
ggplot(transform(ornaments_intact_bell,
                 period = factor(period, 
                                     levels = c("Before European Contact", 
                                                "European Contact", 
                                                "Chinese Contact")))) +
...
ggplot(transform(ornaments_intact_ring,
                 period = factor(period, 
                                     levels = c("Before European Contact", 
                                                "European Contact", 
                                                "Chinese Contact")))) +
...
# plot by period
ggplot(transform(ornaments_period_join_gold,
                 period = factor(period, 
                                     levels = c("Before European Contact", 
                                                "European Contact", 
                                                "Chinese Contact")))) +
  geom_sf(data = sample_units) +
  geom_sf(data = ornaments_period_join_gold, aes(fill = n)) +
  facet_wrap(~ period) +
