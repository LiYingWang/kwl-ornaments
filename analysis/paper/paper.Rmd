...
ggplot() +
  geom_sf(data = sample_units, fill = NA) +
  geom_point(data = ornaments_period_join_cen, 
             aes(x = lon, y = lat))
...
#store x and y coords in two vectors, and create two vectors xrange and yrange with dimensions of triangle that contain all points
lon <- ornaments_period_join_cen$lon
lat <- ornaments_period_join_cen$lat
xrange <- range(lon, na.rm=T)
yrange <- range(lat, na.rm=T)

# create ppp and sp(spacial points) format
ornaments_point_ppp <- ppp(lon, lat, xrange, yrange)
ornaments_point_sp <- as(ornaments_point_ppp, "SpatialPoints")
r_ornaments <- raster(sample_units)

# plot density
den_ornaments <- rasterize(coordinates(ornaments_point_sp), r_ornaments, fun = 'count', background = 0)
plot(den_ornaments)

# kernel density estimation
ds <- density(ornaments_point_ppp)
plot(ds, main = "Ornament density")

kernel_or <- density(ornaments_point_ppp)
kernel_or %>%  # Using the default bandwidth
  plot(main= "Ornament density", las=1)
contour(kernel_or, add=TRUE)
```

To access whether the distribution of ornaments present different density that can indicate the changes caused by global trade network brought by the Europeans, point pattern analysis is used to measure and test the density of ornaments across three time periods.
